    ...
    def extract_parts_from_block(self, image_width, image_height):
        ...
        for block in self.doc.blocks:
            if block.name.startswith('mark_'):
                block_wise_parts_dict[block.name] = {"parts": [], "phase": {}, "image_url": None}
                        ...
                        for virtual_entity in entity.virtual_entities():
                            if virtual_entity.dxftype() == "MTEXT" and re.match(self.parts_regex_pattern, virtual_entity.dxf.text):
                                try:
                                    ...
                                    area = ((length * width) * 2 + (length * thickness) * 2 + (width * thickness) * 2) / 1000000
        ...
        self.logger.info("Parts and phase extraction completed. Generating images...")

        # Generate images for each block after parts extraction
        ig = ImageGenerator(self.doc)
        for block_name in block_wise_parts_dict:
            block_wise_parts_dict[block_name]["image_url"] = ig.generate_image_of_block(block_name=block_name, width=image_width, height=image_height)

        self.logger.info("Successfully generated blockwise parts dict with images.")
